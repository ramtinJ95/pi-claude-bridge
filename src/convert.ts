...
export const PROVIDER_ID = "claude-delegation";
const CLAUDE_CODE_PROVIDER_IDS = new Set([PROVIDER_ID, "claude-bridge"]);

// Pi tool names under Claude Code's builtin names. Only ever correct on the
// DelegateToClaude path, where CC runs its own tools — see mapPiToolNameToSdk.
		...
		if (msg.role === "user") {
			...
			for (const block of content) {
				if (block.type === "text" && block.text) {
					...
					if (CLAUDE_CODE_PROVIDER_IDS.has(msg.provider) && sig) {
