...
- **Refactor: extract the Claude-native delegation engine** — AskClaude now builds Agent SDK options through a pure boundary where isolation and session resume are mutually exclusive by type, and runs through a provider-independent lifecycle that normalizes streamed text, emitted thinking, tool starts/inputs/progress/results with their subagent parent, usage, permission state, retries, rate limits, session metadata, assistant errors, and SDK frames it does not handle. Cancellation now reaches the model as an explicit error result that keeps whatever answer and actions arrived first, and a stream that ends without an authoritative result fails instead of reporting an empty success. The blocking UI stays flat ahead of the richer observability phase.
