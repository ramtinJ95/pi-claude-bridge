...
- **Refactor: extract the Claude-native delegation engine and expose rich AskClaude observability** — AskClaude now builds Agent SDK options through a pure boundary where isolation and session resume are mutually exclusive by type, and runs through a provider-independent lifecycle that normalizes partial streamed text, emitted thinking, tool starts/inputs/progress/results with their subagent parent, usage, permission state, retries, rate limits, session metadata, assistant errors, and SDK frames it does not handle. Its Pi 0.84 tool row streams the same retained snapshot used by the final compact/expanded themed view, including nested tool details, durations, timeline, usage and policy state. Model output, thinking, tool fields and timeline are bounded with visible truncation, and persisted display details receive best-effort credential redaction. The model-facing result spends one total budget in priority order, so a permission override or denial is never the annotation that falls off the end of a capped answer. Cancellation reaches the model as an explicit error result that keeps whatever answer and actions arrived first, and a stream that ends without an authoritative result fails instead of reporting an empty success.
