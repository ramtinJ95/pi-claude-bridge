...
- **Refactor: extract the Claude-native delegation engine** — AskClaude now builds Agent SDK options through a pure boundary and runs through a provider-independent lifecycle that normalizes streamed text, emitted thinking, tool progress/results, usage, permission state, retries, rate limits, session metadata, and unknown SDK events. The existing blocking UI and model-facing result remain unchanged ahead of the richer observability phase.
